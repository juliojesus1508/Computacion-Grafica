...
void OLL::solveOLL(TCube &cube) {
  if (!checkEdges(cube)) {
    int state = findEdges(cube);
    while (state == 0) {
      oll_moves += cube.moves("U");
...
void OLL::solveCase(TCube &cube, int state) {
  switch(state) {
    case 1:
      oll_moves += cube.moves("FRURRRUUUFFF");
      solveOLL(cube);
      break;
    case 2:
      oll_moves += cube.moves("BULUUULLLBBB");
      solveOLL(cube);
      break;
    case 3:
      oll_moves += cube.moves("FRURRRUUUFFFBULUUULLLBBB");
      solveOLL(cube);
      break;
    case 4:
      oll_moves += cube.moves("RURRRURUURRR");
      break;
    case 5:
      oll_moves += cube.moves("RRRUUURUUURRRUUR");
      break;
    case 6:
      oll_moves += cube.moves("FRURRRUUURURRRUUURURRRUUUFFF");
      break;
    case 7:
      oll_moves += cube.moves("RUURRUUURRUUURRUUR");
      break;
    case 8:
      oll_moves += cube.moves("RRDRRRUURDDDRRRUURRR");
      break;
    case 9:
      oll_moves += cube.moves("LFRRRFFFLLLFRFFF");
      break;
    case 10:
      oll_moves += cube.moves("FFFLFRRRFFFLLLFR");
      break;
  }
}

int OLL::findState(TCube &cube) {
  int topLayer[9];
  int redSide[3];
  int greenSide[3];
  int orangeSide[3];
